...
namespace EShop.Shared.Authentication.Managers;
...
public sealed class HttpRequestUserDataProvider : IUserDetailsProvider
{
        ...
        ILogger<HttpRequestUserDataProvider> logger)
    {
        ...
        currentUser = new Lazy<UserData>(() => CreateUserFromRequest(), LazyThreadSafetyMode.PublicationOnly);
    }

    public UserData AuthenticatedUser => currentUser.Value;

    public bool IsAuthenticatedUser => IsSystemUser || TryReadUserDataFromRequest(out _);
    ...
    public bool IsSystemUser
    {
        ...
        {
            UserData? userData;
            if (currentUser.IsValueCreated)
            {
                userData = currentUser.Value;
            }
            else if (TryReadUserDataFromRequest(out var userDataFromToken))
            {
    ...
    {
        _logger.LogDebug("Setting user context to System with no tenant scope [{ProviderHash}]", GetHashCode());
        currentUser = new Lazy<UserData>(() => UserData.GetSystemUser(null), LazyThreadSafetyMode.PublicationOnly);

        var updatedToUser = currentUser.Value ?? throw new InvalidOperationException("User should have been set");
    ...
    public void SetSystemUserContext(string onBehalfOfTenantId, string? onBehalfOfUserId = null, string? onBehalfOfUserType = null)
    {
        ...
        _logger.LogDebug("Setting user context to System (Tenant ID='{TenantId}', User Id='{UserId}') [{ProviderHash}].", onBehalfOfTenantId, onBehalfOfUserId, GetHashCode());
        currentUser =
            string.IsNullOrWhiteSpace(onBehalfOfUserId)
            ? new Lazy<UserData>(() => UserData.GetSystemUser(onBehalfOfTenantId), LazyThreadSafetyMode.PublicationOnly)
            : new Lazy<UserData>(() => UserData.GetSystemUser(onBehalfOfTenantId, onBehalfOfUserId, onBehalfOfUserType), LazyThreadSafetyMode.PublicationOnly);
    ...
    {
        _logger.LogDebug("Resetting user context [{ProviderHash}].", GetHashCode());
        currentUser = new Lazy<UserData>(() => CreateUserFromRequest(), LazyThreadSafetyMode.PublicationOnly);
    ...
    {
        return string.Equals(userId, currentUser.Value.Id, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private UserData CreateUserFromRequest()
    {
        ...
        {
            throw new UnauthorizedException("Could not determine authenticated user from the HTTP request.");
        }
