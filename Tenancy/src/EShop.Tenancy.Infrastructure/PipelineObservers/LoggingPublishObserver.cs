...
public class LoggingPublishObserver : IPublishObserver
{
        ...
        where T : class
    {
        ...
        Log.Error("Fault on publishing message {Message} from {Namespace}, Error: {Error}, CorrelationId: {CorrelationId}", typeof(T).Name, typeof(T).Namespace, exception.Message, context.CorrelationId);
    }
}