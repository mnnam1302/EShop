...
public sealed class VariantUpdatedConsumer : IdempotentConsumer<VariantUpdated>
{
    private readonly IReadModelProjector<Product> _projector;

    public VariantUpdatedConsumer(CatalogReadDbContext dbContext, IReadModelProjector<Product> projector)
        : base(dbContext)
    {
        _projector = projector;
    }

    protected override async Task<Result> HandleMessageAsync(VariantUpdated message, CancellationToken cancellationToken)
    {
