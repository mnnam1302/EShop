...
public sealed class VariantPublishedConsumer : IdempotentConsumer<VariantPublished>
{
    private readonly IReadModelProjector<Product> _projector;

    public VariantPublishedConsumer(CatalogReadDbContext dbContext, IReadModelProjector<Product> projector)
        : base(dbContext)
    {
        _projector = projector;
    }

    protected override async Task<Result> HandleMessageAsync(VariantPublished message, CancellationToken cancellationToken)
    {
