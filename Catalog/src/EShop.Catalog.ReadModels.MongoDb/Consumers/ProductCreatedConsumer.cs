...
public sealed class ProductCreatedConsumer : IdempotentConsumer<ProductCreated>
{
    private readonly IReadModelProjector<Product> _projector;

    public ProductCreatedConsumer(CatalogReadDbContext dbContext, IReadModelProjector<Product> projector)
        : base(dbContext)
    {
        _projector = projector;
    }

    protected override async Task<Result> HandleMessageAsync(ProductCreated message, CancellationToken cancellationToken)
    {
