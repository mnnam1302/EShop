...
public sealed class VariantUnpublishedConsumer : IdempotentConsumer<VariantUnpublished>
{
    private readonly IReadModelProjector<Product> _projector;

    public VariantUnpublishedConsumer(CatalogReadDbContext dbContext, IReadModelProjector<Product> projector)
        : base(dbContext)
    {
        _projector = projector;
    }

    protected override async Task<Result> HandleMessageAsync(VariantUnpublished message, CancellationToken cancellationToken)
    {
