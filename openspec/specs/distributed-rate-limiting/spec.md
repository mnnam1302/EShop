...
The API gateway enforces rate limits that hold consistently across all gateway replicas, using distributed counters backed by Redis with atomic evaluation, tenant/user partitioning, layered tenant+user limits, cached policy resolution with safe fallback, fail-open resilience, shadow-mode rollout per layer, and observability of every decision.
