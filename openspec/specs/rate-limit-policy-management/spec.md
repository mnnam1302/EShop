...
The Tenancy service owns rate-limit policy as part of tenant settings: an optional, validated set of per-domain/per-scope rules, platform defaults stored on the system tenant, a restricted admin endpoint for setting a tenant's policy, and an internal read endpoint the gateway uses to resolve policy with negative caching.
